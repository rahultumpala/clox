    ...
    if (IS_OBJ(callee)) {
        ...
        case OBJ_CLASS: {
            ...
            if(tableGet(&klass->methods, vm.initString, &initializer)) {
                ...
                runtimeErro("Expected 0 arguments but got %d", argCount);
                return false;
            }
            return true;
        }
...
#define BINARY_OP(valueType, op)\
    ...
    for(;;){
        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_SET_GLOBAL: {
                ObjString *name = READ_STRING();
                if(tableSet(&vm.globals, name, peek(0))) { // returns true if inserted as a new key.
                    tableDelete(&vm.globals, name); // delete since implicit var declarations are not supported.
                    runtimeError("Undefined variable '%s'.", name->chars);
                    return INTERPRET_RUNTIME_ERROR;
                }
                break;
